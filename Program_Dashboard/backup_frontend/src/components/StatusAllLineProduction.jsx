        ...
        (Array.isArray(lines) ? lines : Object.values(lines).flat()).map(async (line) => {
          try {
            const [response200, response220] = await Promise.all([
              // axios.get(`http://192.168.127.225:3000/api/actual_total_kwh_panel/tb_total_kwh_pm200?db=${line.db}`),
              // axios.get(`http://192.168.127.225:3000/api/actual_total_kwh_panel/tb_total_kwh_pm220?db=${line.db}`),
              axios.get(`http://172.27.63.180:3000/api/actual_total_kwh_panel/tb_total_kwh_pm200?db=${line.db}`),
              axios.get(`http://172.27.63.180:3000/api/actual_total_kwh_panel/tb_total_kwh_pm220?db=${line.db}`),
            ]);
    ...
    <Container fluid className="p-1">
      <Row className='mt-0'>
        <Col>
          ...
          <p>Note: Kondisi pembacaan aktual power meter yang ditampilkan pada panel genba</p>
        </Col>
      </Row>
          ...
          <Row>
            {entries.map((entry, index) => (
              <Col key={index} xs={12} sm={6} md={4} lg={3} xl={3} className="mb-4">
                <Card className={`h-85 ${entry.status === 'Active' ? 'bg-info bg-opacity-25' : 'bg-danger bg-opacity-25'}`}>
                  <Card.Body>
